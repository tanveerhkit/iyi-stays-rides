...
type RideDetails = { rideType: string; pickup: string; dropoff: string; date: string; time: string; passengers: number; bags: number; vehicleId: string };
...
function RideEditModal({ current, onClose, onSave }: { current: RideDetails; onClose: () => void; onSave: (details: RideDetails) => void }) {
  const [draft, setDraft] = useState(current);
  const selectedVehicle = rideOptions.find((ride) => ride.id === draft.vehicleId) ?? rideOptions[0];
  return <div className="ride-editor-backdrop"><section className="ride-editor-modal"><div className="ride-editor-head"><div><span className="eyebrow-pink">EDIT YOUR RIDE</span><h2>Make the journey<br /><i>fit better.</i></h2></div><button className="sheet-close" onClick={onClose}><X size={20} /></button></div><div className="editor-section"><span className="editor-label">RIDE TYPE</span><div className="editor-ride-types">{rideTypes.map(({ id, label, icon: Icon }) => <button key={id} className={draft.rideType === id ? "selected" : ""} onClick={() => setDraft({ ...draft, rideType: id })}><Icon size={16} /> {label}</button>)}</div></div><div className="editor-fields"><label><MapPin size={16} /><span><small>PICK UP FROM</small><input value={draft.pickup} onChange={(e) => setDraft({ ...draft, pickup: e.target.value })} /></span></label><label><MapPin size={16} /><span><small>DROP OFF AT</small><input value={draft.dropoff} onChange={(e) => setDraft({ ...draft, dropoff: e.target.value })} /></span></label><label><CalendarDays size={16} /><span><small>PICKUP DATE</small><input value={draft.date} onChange={(e) => setDraft({ ...draft, date: e.target.value })} /></span></label><label><Clock3 size={16} /><span><small>PICKUP TIME</small><input value={draft.time} onChange={(e) => setDraft({ ...draft, time: e.target.value })} /></span></label></div><div className="editor-section"><span className="editor-label">CHOOSE A VEHICLE</span><div className="editor-vehicles">{rideOptions.map((ride) => { const Icon = ride.icon; return <button key={ride.id} className={draft.vehicleId === ride.id ? "selected" : ""} onClick={() => setDraft({ ...draft, vehicleId: ride.id })}><Icon size={18} /><span><b>{ride.label}</b><small>{ride.detail}</small></span><strong>{ride.price}</strong></button>; })}</div></div><div className="editor-counters"><Counter label="Guests" value={draft.passengers} onDecrease={() => setDraft({ ...draft, passengers: Math.max(1, draft.passengers - 1) })} onIncrease={() => setDraft({ ...draft, passengers: draft.passengers + 1 })} /><Counter label="Bags" value={draft.bags} onDecrease={() => setDraft({ ...draft, bags: Math.max(0, draft.bags - 1) })} onIncrease={() => setDraft({ ...draft, bags: draft.bags + 1 })} /></div><div className="editor-footer"><span>Estimated total <b>{selectedVehicle.price}</b></span><button className="primary-cta" onClick={() => onSave(draft)}>Save ride <Check size={17} /></button></div></section></div>;
}
